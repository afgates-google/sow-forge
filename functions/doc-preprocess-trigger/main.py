...
def doc_preprocess_trigger(cloud_event): # CORRECTED NAME
    ...
    try:
        if 'message' in cloud_event.data and 'data' in cloud_event.data['message']:
            ...
            print("Received manual Pub/Sub trigger.")
        else:
            event_data = cloud_event.data
            print("Received GCS event trigger from Eventarc.")
        bucket_name, file_name = event_data["bucket"], event_data["name"]
    except Exception as e:
        print(f"!!! Could not parse event data: {e}. Payload: {cloud_event.data}")
        return
    ...
    try:
        ...
        error_message = f"!!! CRITICAL ERROR in doc_preprocess_trigger: {e}"
